...
bool GMMDesc::operator==(GMMDesc const& rhs) const
{
	return (
			// check size
...
std::ostream& operator<<(std::ostream& os, GMMDesc const& desc)
{
	...
	{	
		...
		for(std::size_t i=0; i<k; ++i)
		{
			os << i+1 << ". weight = " << desc.weights[i] << std::endl
				...
				<< "covariance:" << std::endl << desc.covariances[i] << std::endl << std::endl;
		}
	}
	else
	{
